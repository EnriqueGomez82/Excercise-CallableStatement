...
public class Act3_4 {
    ...
    public static void main(String[] args) {
        ...
        // count_Teacher_By_Department();             // BUSCA PROFESORES POR DEPARTAMENTO 
        //exit();                                      // TE PREGUNTA SI QUIERES SALIR DEL PROGRAMA
        //consultaDB();                                   // HACE UNA CONSULTA A LA BASE DE DATOS
        // cambio_Nombre_Procedure();                   // PROCEDIMIENTO QUE CAMBIA EL NOMBRE A UN DEPARTAMENTO
        // cambio_Nombre();                             // CAMBIA EL NOMBRE AL DEPARTAMENTO ELEGIDO POR EL USUARIO
        // cambio_Profesor_De_Departamento_Procedure(); // PROCEDIMIENTO PARA CAMBIAR A UN PROFESOR DE DEPARTAMENTO
        //  cambio_Profesor_De_Departamento();         // SELECCIONA UN PROFESOR POR ID Y LE CAMBIA EL DEPARTAMENTO
    ...
    public static void count_Teacher_By_Department_Procedure() {
        try {
            ...
            sql = "CREATE PROCEDURE count_Teacher_By_Department_Procedure(OUT numero int, nombre Varchar(20))"
                    + "READS sql DATA "
                    + "BEGIN ATOMIC "
                    + "SET numero = (SELECT COUNT(id) FROM teachers WHERE dept_num IN (SELECT dept_num FROM departments WHERE name=nombre)); " // pasar al set el parametro out
    ...
    public static void count_Teacher_By_Department() {
        try {
            ...
            System.out.println("¿Que departamento quieres buscar?\nLos departamentos existentes son:\nfisica\ninformatica\nfol\nbiologia\narquitectura\narqueologia\nprogramacion");
            String dep = tcl.nextLine();

            call.registerOutParameter(1, Types.INTEGER); // pasar el primer parametro out y definir su tipo
            call.setString(2, dep);

            call.execute();
            System.out.println("\nEl total de profesores es: " + call.getString(1) + "!!!\n");
            con.close();
    ...
    public static void cambio_Nombre_Procedure() {
        try {
            ...
            String sql = "DROP PROCEDURE cambio_Nombre_Procedure IF EXISTS;";
            stt.executeUpdate(sql);

            sql = "CREATE PROCEDURE cambio_Nombre_Procedure(nombre varchar(20), numero int) "
                    + "modifies sql data "
                    + "BEGIN ATOMIC "
                    + "UPDATE departments SET name = nombre WHERE dept_num=numero; "
                    + "END";
    ...
    }// CREA PROCEDIMIENTO PARA CAMBIAR EL NOMBRE A UN DEPARTAMENTO

    public static void cambio_Nombre() {
        try {
            Scanner tcl = new Scanner(System.in);
            Scanner sc = new Scanner(System.in);
            Connection con = conexion("jdbc:hsqldb:.\\database\\db");
            Statement stt;
            stt = con.createStatement();
            CallableStatement call = con.prepareCall(" call cambio_Nombre_Procedure(?,?)");
            System.out.println("Dime el ID del departamento que quieres cambiar el nombre");
            int id = sc.nextInt();
            System.out.println("¿Cual es el nuevo nombre del departamento?");
            String dep = tcl.nextLine();
    ...
    } // CAMBIA EL NOMBRE DE UN DEPARTAMENTO POR ID

    public static void cambio_Profesor_De_Departamento_Procedure() {
        ...
        try {
            ...
            String sql = "DROP PROCEDURE cambio_Profesor_De_Departamento_Procedure IF EXISTS;";
            stt.executeUpdate(sql);

            sql = "CREATE PROCEDURE cambio_Profesor_De_Departamento_Procedure(num int, numero int) "
                    + "modifies sql data "
                    + "BEGIN ATOMIC "
                    + "UPDATE teachers SET dept_num = numero WHERE id=num; "
                    + "END";
    ...
    } // PROCEDIMIENTO PARA CAMBIAR A UN PROFESOR DE DEPARTAMENTO

    public static void cambio_Profesor_De_Departamento() {
        try {
            Scanner tcl = new Scanner(System.in);
            Scanner sc = new Scanner(System.in);
            Connection con = conexion("jdbc:hsqldb:.\\database\\db");
            Statement stt;
            stt = con.createStatement();
            CallableStatement call = con.prepareCall(" call cambio_Profesor_De_Departamento_Procedure(?,?)");
            System.out.println("Dime el ID del profesor que quieres cambiar el departamento");
            int num = sc.nextInt();
    ...
    } // SELECCIONA UN PROFESOR POR ID Y LE CAMBIA EL DEPARTAMENTO 
    ...
    public static void menu() {
        ...
        String bienvenida = ("BIENVENIDO AL EJERCICIO MÁS LARGO DE LA HISTORIA!!!\n"
                ...
                + "\n7. Count teacher of department...\n8. Do query to the database\n9. Change the name of a department by id"
                + "\n10. Change a teacher of their dept_num\n11. CREATE ALL PROCEDURES AGAIN\n12. QUIT APP");
        ...
        while (op != 12) {
            ...
            switch (op) {
                ...
                case 9:
                    ...
                    break;
                case 10:
                    cambio_Profesor_De_Departamento();
                    break;
                case 11:
                    newDepartmentProcedure();
                    newTeacherProcedure();
                    set_SalaryProcedure();
                    salary_Porcentaje_Procedure();
                    get_Newest_Teacher2_Procedure();
                    count_Teacher_By_Department_Procedure();
                    cambio_Nombre_Procedure();
                    cambio_Profesor_De_Departamento_Procedure();
                    System.out.println("HAS CREADO TODOS LOS PROCEDIMIENTOS DE GOLPE OTRA VEZ!!! STILL WORKING :)");
                    break;
                case 12:
                    exit();
                    break;
